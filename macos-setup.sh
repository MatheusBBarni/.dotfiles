...
parse_args() {
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --skip)
        if [[ $# -lt 2 ]]; then
          ui_error "--skip requires a comma-separated component list"
          exit 1
        fi
        ...
        for component in "${requested_components[@]}"; do
          ...
          if skip_component_supported "$component"; then
            ...
            ui_error "Unknown --skip component: $component"
            ui_info "Supported components: ${SKIP_COMPONENTS[*]}"
            exit 1
          fi
        done
        shift 2
...
run_optional() {
  ...
  if skip_requested "$component"; then
    ui_info "Skipping $component (--skip)"
    return 0
  fi
  "$@"
}
...
configure_dock() {
  ui_step "Configuring Dock"

  if ! command -v dockutil >/dev/null 2>&1; then
    ui_warn "dockutil not found; skipping Dock setup"
    return
  fi
  ...
  for app in "${DOCK_APPS[@]}"; do
    if [[ -d "$app" ]]; then
      ...
      ui_warn "Dock app not found; skipping: $app"
    fi
  done
...
install_oh_my_zsh() {
  ...
  if ! grep -qxF "$brew_zsh" /etc/shells; then
    ui_info "Adding Homebrew zsh to /etc/shells"
    echo "$brew_zsh" | sudo tee -a /etc/shells >/dev/null
  fi

  if [[ "$SHELL" != "$brew_zsh" ]]; then
    ui_info "Setting Homebrew zsh as the default shell"
    chsh -s "$brew_zsh"
  fi
...
install_global_bun_packages() {
  ...
  if ! command -v bun >/dev/null 2>&1; then
    ui_warn "Bun not found; skipping global packages"
    return 0
  fi

  if skip_requested pi; then
    ui_info "Skipping pi (--skip)"
  elif command -v pi >/dev/null 2>&1; then
    ui_success "Already installed: pi"
  else
    bun add -g @earendil-works/pi-coding-agent
  fi

  if skip_requested opencode; then
    ui_info "Skipping opencode (--skip)"
  elif command -v opencode >/dev/null 2>&1; then
    ui_success "Already installed: opencode"
  else
...
install_bettervim() {
  ...
  if [[ -z "$BETTERVIM_LICENSE" ]]; then
    ui_info "bettervim license not provided; skipping"
    return 0
  fi
...
install_java_kotlin() {
  ui_step "Installing Java and Kotlin"
...
install_handy() {
  ...
  if [[ -d "/Applications/Handy.app" ]]; then
    ui_success "Already installed: Handy"
    return 0
  fi
  ...
  if [[ -z "$app_path" ]]; then
    ...
    ui_error "Handy.app was not found in the mounted DMG"
    return 1
  fi
...
install_xcode() {
  ui_step "Installing Xcode"

  if [[ ! -d "/Applications/Xcode.app" ]]; then
    if mas account >/dev/null 2>&1; then
      mas install 497799835 || {
        ui_error "Xcode install failed. Open the App Store, sign in, and install Xcode manually."
        return 0
      }
    else
      ui_warn "Mac App Store is not signed in. Install Xcode manually from the App Store."
      return 0
    fi
  fi
...
if skip_requested bun && ! have_cmd omp; then
  ui_info "Skipping omp because bun was skipped"
else
